...
# Checking if it's a container database or not
if [[ "${CONTAINER_DB}" == "false" ]]; then
  sed -i -e "s|createAsContainerDatabase=true|createAsContainerDatabase=false|g" "$ORACLE_BASE"/dbca.rsp
  sed -i -e "s|^numberOfPDBs=|#numberOfPDBs=|g" "$ORACLE_BASE"/dbca.rsp
  sed -i -e "s|^pdbName=|#pdbName=|g" "$ORACLE_BASE"/dbca.rsp
  sed -i -e "s|^pdbAdminPassword=|#pdbAdminPassword=|g" "$ORACLE_BASE"/dbca.rsp
fi
...
if [[ "${CLONE_DB}" == "true" ]] || [[ "${STANDBY_DB}" == "true" ]]; then
  ...
  # # Adding DB_FILE_NAME_CONVERT initParam if provided
  # if [ -n "${DB_FILE_NAME_CONVERT}" ]; then
  #   sed -i -e "s|initParams=.*|&,DB_FILE_NAME_CONVERT=${DB_FILE_NAME_CONVERT}|g" "$ORACLE_BASE"/dbca.rsp
  # fi

  # # Adding LOG_FILE_NAME_CONVERT initParam if provided
  # if [ -n "${LOG_FILE_NAME_CONVERT}" ]; then
  #   sed -i -e "s|initParams=.*|&,LOG_FILE_NAME_CONVERT=${LOG_FILE_NAME_CONVERT}|g" "$ORACLE_BASE"/dbca.rsp
  # fi

  # Adding DATAFILE_DESTINATION initParam if provided
  if [ -n "${DATAFILE_DESTINATION}" ]; then
    DESTINATION_CONFIG="-datafileDestination ${DATAFILE_DESTINATION}"
    INITPARAMS="-initParams db_create_file_dest=${DATAFILE_DESTINATION}"
  fi

  if [ -n "${RECOVERY_AREA_DESTINATION}" ]; then
    DESTINATION_CONFIG="${DESTINATION_CONFIG} -recoveryAreaDestination ${RECOVERY_AREA_DESTINATION}"
    INITPARAMS=$(echo ${INITPARAMS} | sed -e "s|-initParams .*|&,db_recovery_file_dest=${RECOVERY_AREA_DESTINATION}|g")
  fi
  ...
  if [ "${STANDBY_DB}" = "true" ]; then
    ...
    dbca -silent -createDuplicateDB -gdbName "$PRIMARY_DB_NAME" -primaryDBConnectionString "$PRIMARY_DB_CONN_STR" ${DBCA_CRED_OPTIONS} -sid "$ORACLE_SID" -createAsStandby -dbUniquename "$ORACLE_SID"  ${DESTINATION_CONFIG} ${INITPARAMS} ORACLE_HOSTNAME="$ORACLE_HOSTNAME" ||
